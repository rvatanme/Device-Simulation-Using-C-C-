...
Since, in DD model algorithm, there are several coupling linear algebraic equations, an iterative method should be used in order to solve them together. The two popular methods for solving the coupled discretized equations are the Gummel's iteration method<sup>1</sup> (used in the code) and the Newton's method. 
...
## References
1) Drift-Diffusion Model: Solution Details,  Dragica Vasileska, Arizona State University (https://nanohub.org/resources/1565/download/ddmodel_solution_details_word.pdf)
